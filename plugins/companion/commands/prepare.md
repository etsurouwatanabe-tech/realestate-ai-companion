...
`prepare.py` は既存データを保持しながら新規都道府県分を追加します：

- `/companion:prepare 東京` → 東京分を取得（既存に神奈川等があれば残す）
- 同じ都道府県を再 prepare すれば最新化（地価年次更新時など）
- 完全リセットしたい場合は `/companion:prepare 東京 --reset`

ユーザーには「東京だけまず試してから、欲しくなったら神奈川を追加すればいい」と案内可。
